...
\section{数据处理与分析}
\subsection{图像强度提取方法}
CCD 图像为绿色干涉光图像，因此取 RGB 图像的绿色通道灰度值作为相对光强 $I$。对每幅图像先用亮度阈值确定有效光斑区域，再取有效光斑中部水平带作为分析区域。分析区域的纵向范围记为 $y_0\sim y_1$，横向范围记为 $x_0\sim x_1$。在该区域内沿 $y$ 方向求平均，得到横向一维平均强度剖面
\begin{equation}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{cccccccc}
        ...
        \midrule
        (a) & 114--1082 & 361--575 & 0.570 & 0.439 & 0.511 & 0.945 & 454 \\
        (b) & 220--1170 & 301--531 & 0.208 & 0.234 & 0.575 & 0.710 & 920 \\
        (c) & 24--1252 & 416--674 & 0.141 & 0.130 & 0.096 & 0.236 & 244 \\
        (d) & 30--1235 & 364--646 & 0.113 & 0.069 & 0.066 & 0.160 & 330 \\
        (e) & 28--1252 & 380--668 & 0.099 & 0.086 & 0.086 & 0.153 & 228 \\
        (f) & 26--1258 & 384--668 & 0.103 & 0.082 & 0.082 & 0.149 & 266 \\
        \bottomrule
    \end{tabular}
\end{table}

从表~\ref{tab:visibility-results} 可见，图像 (a)、(b) 的局部最大可见度分别达到 0.945 和 0.710，说明其分析区域内存在较清晰的条纹；图像 (c)--(f) 的最大可见度降至 0.236、0.160、0.153 和 0.149，条纹对比度明显降低，图像中散斑和光强起伏占主要成分。该结果与直接观察图像得到的现象一致：部分实验条件下干涉条纹较清楚，而另一些条件下条纹被显著削弱。

图像 (a) 的较高可见度区域出现在 $x\approx454$ pixel 附近；图像 (b) 的较高可见度区域出现在 $x\approx920$ pixel 附近。二者最大值位置相差约 $466$ pixel，表明改变毛玻璃状态或位置后，满足较强空间相干条件的区域发生了明显横向移动。图像 (c)--(f) 的可见度整体较低，但其局部最大值仍主要出现在左侧区域，说明在这些记录条件下只有较小范围内仍保留较弱条纹。

\subsection{不同区域条纹对比度差异}
实验图像中，条纹并非均匀分布在整个重叠光斑中。由表~\ref{tab:visibility-results} 可知，各图像左、中、右三个区域的平均可见度不同。例如图像 (b) 的右区平均可见度为 0.575，明显高于左区的 0.208 和中区的 0.234；图像 (d)--(f) 的三个区域平均可见度均低于 0.12，说明条纹只以较弱形式存在。
...
改变毛玻璃位置会改变部分相干光源的等效源面位置及其经透镜后的成像关系，进而改变 CCD 平面上两束干涉光对应源面点的横向间距分布。图像 (a) 与 (b) 的 $V_{\max}$ 位置分别位于 $x=454$ pixel 与 $x=920$ pixel，明显条纹区域的位置发生横向移动；图像 (c)--(f) 中 $V_{\max}$ 下降到 0.24 以下，表明相应条件下有效相干区域明显缩小或条纹对比度整体下降。因此，图像数据支持“毛玻璃位于不同位置时条纹出现区域会变化”的判断。
...
\begin{enumerate}
    ...
    \item 由图像提取的局部条纹可见度显示，图像 (a)、(b) 的最大可见度分别为 0.945 和 0.710，而图像 (c)--(f) 的最大可见度低于 0.24，说明不同实验条件下条纹对比度差异显著。
    \item 图像 (a)、(b) 的最大可见度位置分别为 $x=454$ pixel 和 $x=920$ pixel，说明毛玻璃位置或状态改变会改变相干性较好区域在 CCD 平面上的位置。
\end{enumerate}
