...
\section{实验过程}
\begin{enumerate}
    \item 按实验文档的 $\beta$-$\gamma$ 符合装置框图连接系统（见图~\ref{fig:setup-scheme}）。
    \item 用精密脉冲发生器作为输入，调节放大器增益、成形时间、单道阈值和符合延迟，使符合通道稳定输出。
    \item 固定一路延迟，连续扫描另一路延迟，在示波器上观察符合输出波形并粗调工作区间。
...
注：原记录中 $t=\SI{3.3}{\micro\second}$ 处记为 0，视为无效点，不参与统计。
...
第一组由标准脉冲发生器输入测得，用于在实验中确定符合电路延迟旋钮的粗调区间，并保证后续真实信号扫描覆盖有效符合区域。
...
\begin{axis}[
    ...
    xlabel={绝对延迟时间 $t$ (\si{\micro\second})},
    ylabel={计数 $N$（10 s）},
    xmin=3.35, xmax=5.35,
...
\end{axis}
\end{tikzpicture}
\caption{第一组标准信号粗调延迟平台图}\label{fig:group1-platform}
\end{figure}
